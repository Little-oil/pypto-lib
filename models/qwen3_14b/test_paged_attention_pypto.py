...
compiler-pass snapshots, and Step-1 through Step-8 evidence protocols.  Qwen
decode now has one production backend, so this file intentionally keeps only
the checks that can still catch a product regression:
...
def paged_attention_pypto_dynamic(
    ...
    """Allocate fused Phase-0/PA scratch and launch the production helper."""
...
def _apply_pattern(
    ...
    if case.pattern == "kv-head-id":
        key[active_pages].zero_()
        for kv_head in range(NUM_KV_HEADS):
            value[active_pages, :, kv_head].fill_(float(kv_head + 1))
...
def test_production_pa_clamps_warmup_slot_before_layer_base() -> None:
    source = Path(__file__).with_name("paged_attention_pypto.py").read_text()
...
def test_production_pa_preserves_two_stack_prelaunch_skew() -> None:
    source = Path(__file__).with_name("paged_attention_pypto.py").read_text()
    assert PRE_LAUNCH == 2
