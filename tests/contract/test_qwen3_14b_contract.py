...
def _run_change_detector(*changed: str, flag: str | None = None) -> str:
    command = [sys.executable, ".github/scripts/detect_changes.py"]
    if flag is not None:
        command.append(flag)
...
def test_step7_change_detection_routes_qwen_pa_only_to_explicit_jobs() -> None:
    explicit = set(_run_change_detector(flag="--list-explicit-runnables").split())
    assert {
        "models/qwen3_14b/decode_fwd.py",
        "models/qwen3_14b/test_paged_attention_pypto.py",
    } <= explicit
    assert (
        _run_change_detector(
            "models/qwen3_14b/paged_attention_pypto.py",
            flag="--qwen-pa-impact",
        )
        == "true"
    )
    assert _run_change_detector("docs/models/qwen3.md", flag="--qwen-pa-impact") == "false"
    generic = set(_run_change_detector("models/qwen3_14b/paged_attention_pypto.py").split())
    assert generic.isdisjoint(explicit)


def test_step9_ci_has_dedicated_a2a3_only_pypto_lanes() -> None:
    workflow = (_REPO_ROOT / ".github" / "workflows" / "ci.yml").read_text()
    sim_start = workflow.index("  qwen-pa-a2a3sim:")
    sim_end = workflow.index("\n  qwen-pa-device:", sim_start)
    sim = workflow[sim_start:sim_end]
    assert "-p a2a3sim" in sim
    assert "--matrix pr --compile-only" in sim
    assert "decode_fwd.py" in sim
    assert "PYPTO_QWEN3_PA_IMPL" not in sim
    assert "expect-pa-impl" not in sim
    assert "legacy_cce" not in sim
    assert "a5sim" not in sim

    device_start = workflow.index("  qwen-pa-device:")
    device_end = workflow.index("\n  serving-qwen:", device_start)
    device = workflow[device_start:device_end]
    assert "--matrix pr" in device
    assert "test_paged_attention_pypto.py" in device
    assert "decode_fwd.py" in device
    assert "--validate-fwd --fwd-layers 1 -b 17" in device
    assert "PYPTO_QWEN3_PA_IMPL" not in device
    assert "expect-pa-impl" not in device
    assert "legacy_cce" not in device

    assert "qwen-pa-release-manifest" not in workflow
    assert "qwen-pa-frozen-ab" not in workflow
    assert "paged_attention_legacy_oracle_v1.json" not in workflow


def test_step9_daily_and_serving_automation_are_sole_pypto() -> None:
    daily = (_REPO_ROOT / ".github" / "workflows" / "daily_ci.yml").read_text()
    full_start = daily.index("  qwen-pa-full:")
    full_end = daily.index("\n  model-tests-a5:", full_start)
    full = daily[full_start:full_end]
    assert "--matrix full" in full
    assert "test_paged_attention_pypto.py" in full
    assert "decode_fwd.py" in full
    assert "--validate-fwd --fwd-layers 1 -b 17" in full
    assert "PYPTO_QWEN3_PA_IMPL" not in full
    assert "expect-pa-impl" not in full
    assert "legacy_cce" not in full
    assert "PYPTO_BENCH" not in full
    assert daily.count("--list-explicit-runnables") == 2

    workflow = (_REPO_ROOT / ".github" / "workflows" / "ci.yml").read_text()
    serving_start = workflow.index("  serving-qwen:")
    serving_end = workflow.index("\n  serving-deepseek:", serving_start)
    serving = workflow[serving_start:serving_end]
    assert "strategy:" not in serving
    assert "qwen3-pa-impl" not in serving

    action = (_REPO_ROOT / ".github" / "actions" / "pypto-serving-tests" / "action.yml").read_text()
    assert "qwen3-pa-impl" not in action
    assert "PYPTO_QWEN3_PA_IMPL" not in action
    assert "validate_pa_impl_selection" not in action
    assert "print_pa_backend" not in action
    assert "export PYPTO_QWEN3_MODEL_DIR=$PYPTO_QWEN3_MODEL_DIR" in action
    assert "export PTO2_RING_DEP_POOL=$PTO2_RING_DEP_POOL" in action
