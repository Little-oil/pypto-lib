...
def o_group_a2a_with_completion(
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="o_group_a2a_wait", allow_early_resolve=True, deps=[publish_dep]) as wait_tid:
    ...
    local_groups_out, gather_tid = pl.spmd_submit(
        ...
        core_num=ATTENTION_PUBLISH_WORKERS, allow_early_resolve=True, deps=[wait_tid],
    )

    with pl.at(level=pl.Level.CORE_GROUP, name_hint="o_group_a2a_complete", allow_early_resolve=True, deps=[gather_tid], no_dep_args=[exchange_signal]) as completion_tid:
...
def o_proj_reduce_scatter(
    ...
    for owner in pl.parallel(TP_SIZE):
        ...
        for local_group in pl.parallel(LOCAL_O_GROUPS):
            ...
            with pl.spmd(O_A_QUANT_WORKERS, name_hint="tp_o_a_quant", allow_early_resolve=True, deps=[pa_tid]) as q_tid:
            ...
            with pl.spmd(own_b_rows * (D // O_B_D_TILE), name_hint="tp_o_b", allow_early_resolve=True, deps=[q_tid]):
                pb_unit = pl.tile.get_block_idx()
        ...
        with pl.spmd(
            ...
            name_hint="tp_o_b_dequant", allow_early_resolve=True,
